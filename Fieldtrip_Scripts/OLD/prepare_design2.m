function [VecPred,IND,timelockbaset] = prepare_design2(nip,chansel,source,segwin,latency)
...
TD_Fut_quartiled = [ones(2,6)*NaN ; TD_Fut_quartiled];
...
TD_Pas_quartiled = [TD_Pas_quartiled ; ones(2,6)*NaN];
...
TD_W_quartiled = [TD_W_quartiled  ones(6,2)*NaN];
...
TD_E_quartiled = [ones(6,2)*NaN  TD_E_quartiled];
...
SD_W_quartiled = [SD_W_quartiled ones(6,2)*NaN];
...
SD_E_quartiled = [ones(6,2)*NaN  SD_E_quartiled];
...
SD_Pas_quartiled = [SD_Pas_quartiled ; ones(2,6)*NaN];
...
SD_Fut_quartiled = [ones(2,6)*NaN ; SD_Fut_quartiled];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% prepare distance predictors
clear DistTPred DistSPred
for i =1:length(trldef)
    tmp = [];tmp = num2str(trldef(i,4));
    for j = 1:36
        if str2num(tmp([1 2])) == EVENT_ID(j)
            if REFpred(i) == 1
                DistTPred(i) = TD_Pre_quartiled(j);
                DistSPred(i) = SD_Par_quartiled(j);
            elseif REFpred(i) == 2
                DistTPred(i) = TD_Pas_quartiled(j);
                DistSPred(i) = SD_Pas_quartiled(j);
            elseif REFpred(i) == 3
                DistTPred(i) = TD_Fut_quartiled(j);
                DistSPred(i) = SD_Fut_quartiled(j);
            elseif REFpred(i) == 4
                DistTPred(i) = TD_W_quartiled(j);
                DistSPred(i) = SD_W_quartiled(j);
            elseif REFpred(i) == 5
                DistTPred(i) = TD_E_quartiled(j);
                DistSPred(i) = SD_E_quartiled(j);
            end
        end
    end
end
