...
TABLE{1,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*60   + repmat([1;3],1,36);
TABLE{2,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*80   + repmat([1;3],1,36);
TABLE{3,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*100 + repmat([1;3],1,36);
TABLE{4,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*120 + repmat([1;3],1,36);
TABLE{5,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*140 + repmat([1;3],1,36);
TABLE{6,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*70   + repmat([1;3],1,36);
TABLE{7,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*90   + repmat([1;3],1,36);
TABLE{8,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*110 + repmat([1;3],1,36);
TABLE{9,2}   = repmat(37000:1000:72000,2,1) + ones(2,36)*130 + repmat([1;3],1,36);
TABLE{10,2}  = repmat(37000:1000:72000,2,1) + ones(2,36)*150 + repmat([1;3],1,36);
...
for i = 1:size(TABLE,1)
    ...
    while j <= length(TABLE{i,2}(:))
        ...
        if isempty(get_filenames(FOLDER,'QT',num2str(TABLE{i,2}(j)))) == 0
            NAMESLIST{i}{k} = get_filenames(FOLDER,'QT',num2str(TABLE{i,2}(j)),'EEG');
