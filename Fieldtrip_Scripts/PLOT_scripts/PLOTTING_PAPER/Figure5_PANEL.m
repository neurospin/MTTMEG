...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% DIST TIME %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
condnames_clust = {'EtPreG';'EtPastG';'EtFutG';'EtWestG';'EtEastG'};
condnames    = {'EtPreG';'EtPastG';'EtFutG';'EtWestG';'EtEastG'};
latency      = [0 1];
...
clustnum     = 0;
clusttype    = 'posclust';
timetag      = '201672514151';

[ch_mag, cdn_mag, cdn_clust_mag, stat_mag, GDAVG_mag, GDAVGt_mag] = prepare_comp(niplist,condnames,...
    condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)
...
clustnum     = 0;
clusttype    = 'posclust';
timetag      = '201672514250';

[ch_grad22, cdn_grad22, cdn_clust_grad22, stat_grad22, GDAVG_grad22, GDAVGt_grad22] = prepare_comp(niplist,condnames,...
    condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)
...
[ch_grad2, cdn_grad2, cdn_clust_grad2, stat_grad2, GDAVG_grad2, GDAVGt_grad2] = prepare_comp(niplist,condnames,...
    condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)
...
lay                        = ft_prepare_layout(cfg,GDAVGt_mag{1});
lay.label                = GDAVGt_mag{1}.label;
...
lay                        = ft_prepare_layout(cfg,GDAVGt_grad2{1});
lay.label                = GDAVGt_grad2{1}.label;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
times = sum(stat_grad2.prob <= 0.05);
plot(stat_grad2.time,times)
[x,y] = findpeaks(times);
...
for index = y
    ...
    cfg.highlightchannel   = stat_grad2.label(indchan);
    cfg.highlightsymbol    = '.';
...
[x,y]    = findpeaks(times);
indchan1 = find(stat_eeg.prob(:,y(6)) <0.05);
clust1   = find(sum(stat_eeg.prob <= 0.05) > 0);

times    = sum(stat_mag.prob <= 0.05);
plot(stat_mag.time,times)
[x,y]    = findpeaks(times);
indchan2 = find(stat_mag.prob(:,y(11)) <0.05);
clust2   = find(sum(stat_mag.prob <= 0.05) > 0);

times    = sum(stat_mag.prob <= 0.05);
plot(stat_mag.time,times)
[x,y]    = findpeaks(times);
indchan3 = find(stat_mag.prob(:,y(22)) <0.05);
clust3   = find(sum(stat_mag.prob <= 0.05) > 0);

times    = sum(stat_grad2.prob <= 0.05);
plot(stat_grad2.time,times)
[x,y]    = findpeaks(times);
indchan4 = find(stat_grad2.prob(:,y(1)) <0.05);
clust4   = find(sum(stat_grad2.prob <= 0.05) > 0);

times    = sum(stat_grad22.prob <= 0.05);
plot(stat_grad22.time,times)
[x,y]    = findpeaks(times);
indchan5 = find(stat_grad22.prob(:,y(2)) <0.05);
clust5   = find(sum(stat_grad22.prob <= 0.05) > 0);
...
set_axes(-0.7, 2, -1,2.8)

% stimulus onsets
line([0 0]    ,[-5 5],'linestyle',':','linewidth',5,'color',[0.5 0.5 0.5]); hold on
line([1 1],[-5 5],'linestyle',':','linewidth',5,'color',[0.5 0.5 0.5]); hold on
line([1.8 1.8],[-5 5],'linestyle',':','linewidth',5,'color',[0.5 0.5 0.5]); hold on

% cluster shade
% cmap = colormap('gray')
% cmap_inv = cmap(end:-1:1,:)
% plot_lin_shade_v2(clust1 ,0.05 , stat_eeg ,-6.5,5,[0.7 0.7 0.7],-0.8,indchan1,cmap_inv,[0 5])
% plot_lin_shade_v2(clust2 ,0.05 , stat_magt ,-6.5,5,[0.7 0.7 0.7],-7,indchan_2,cmap,[-5 5])

% timecourse
plot(GDAVGt_eeg{1}.time,mean(GDAVG_eeg{4}.avg(indchan1,:)*fmult),'linewidth',8,'color',graphcolor(4,:));hold on
plot(GDAVGt_eeg{1}.time,mean(GDAVG_eeg{5}.avg(indchan1,:)*fmult),'linewidth',8,'color',graphcolor(5,:));hold on
plot(GDAVGt_eeg{1}.time,mean(GDAVG_eeg{1}.avg(indchan1,:)*fmult),'linewidth',8,'color',graphcolor(1,:));hold on
plot(GDAVGt_eeg{1}.time,mean(GDAVG_eeg{2}.avg(indchan1,:)*fmult),'linewidth',8,'color',graphcolor(2,:));hold on
plot(GDAVGt_eeg{1}.time,mean(GDAVG_eeg{3}.avg(indchan1,:)*fmult),'linewidth',8,'color',graphcolor(3,:));hold on
