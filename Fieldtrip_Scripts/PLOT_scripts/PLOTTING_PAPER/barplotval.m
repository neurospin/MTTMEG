function data = barplotval(GDAVGt,alpha,stat,index,graphcolor,limaxis,fmult)
...
elseif length(t) == 2
    ...
    dat1 = mean(mean(GDAVGt{1}.individual(:,indchan,interval),2),3)*fmult;
    dat2 = mean(mean(GDAVGt{2}.individual(:,indchan,interval),2),3)*fmult;
    dat3 = mean(mean(GDAVGt{3}.individual(:,indchan,interval),2),3)*fmult;
