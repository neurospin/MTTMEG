...
for index = y
    ...
    cfg.marker             = 'off';
    cfg.highlight          = 'on';
    indchan = []; indchan  = find(stat_magt.prob(:,index) <0.025);
...
[x,y]    = findpeaks(times);
indchan2 = find(stat_magt2.prob(:,y(8)) <0.025);
indchanbest2 = find(abs(stat_magt2.stat(:,y(4))) == max(abs(stat_magt2.stat(:,y(4)))))
...
stat_space = load(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/spatial_clust_from_mne_unsig/' ...
    'stats_clust_T_RelWestG_RelEastG__Mags_Lat0-1_stimlock_201692217827.mat'],'stat')
