...
latency      = [0 1];
graphcolor   = [[1 0.6 0.6];[1 0 0];[1 0.6 0.6];[1 0 0]];
stat_test    = 'T';
chansel      = 'Mags';
clustnum     = 0;
clusttype    = 'posclust';
...
[ch_magii, cdn1_magii, cdn2_magii, cdn_clust_magii, stat_magii, GDAVG_magii, GDAVGt_magii] = prepare_comp_INT(niplist,condnames1,...
...
[ch_magii2, cdn1_magii2, cdn2_magii2, cdn_clust_magii2, stat_magii2, GDAVG_magii2, GDAVGt_magii2] = prepare_comp_INT(niplist,condnames1,...
    condnames2,condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)
...
[ch_magiii2, cdn1_magiii2, cdn2_magiii2, cdn_clust_magiii2, stat_magiii2, GDAVG_magiii2, GDAVGt_magiii2] = prepare_comp_INT(niplist,condnames1,...
    condnames2,condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
condnames_clust = {'RelPastG_intmap','RelFutG_intmap'};
...
clustnum     = 0;
clusttype    = 'negclust';
timetag      = '201633118352';

[ch_magt1, cdn_magt1, cdn_clust_magt1, stat_magt1, GDAVG_magt1, GDAVGt_magt1] = prepare_comp(niplist,condnames,...
    condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
condnames_clust = {'RelPastG_coumap','RelFutG_coumap'};
...
clustnum     = 0;
clusttype    = 'negclust';
timetag      = '2016331183620';

[ch_magt2, cdn_magt2, cdn_clust_magt2, stat_magt2, GDAVG_magt2, GDAVGt_magt2] = prepare_comp(niplist,condnames,...
    condnames_clust, latency, graphcolor , stat_test, chansel, clustnum, clusttype, timetag)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
condnames_clust = {'RelPastG','RelFutG'};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
condnames_clust = {'RelPastG','RelFutG','RelWestG','RelEastG'};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
times = sum(stat_magt2.prob <= 0.025);
...
for index = y
    ...
    cfg.marker             = 'off';
    cfg.highlight          = 'on';
    indchan = []; indchan  = find(stat_magt2.prob(:,index) <0.025);
    cfg.highlightchannel   = stat_magt2.label(indchan);
    cfg.highlightsymbol    = '.';
...
[x,y]    = findpeaks(times);
indchan1 = find(stat_magt.prob(:,y(1)) <0.025);
clust1   = find(sum(stat_magt.prob <= 0.025) > 0);

times    = sum(stat_magt2.prob <= 0.025);
plot(stat_magt2.time,times)
[x,y]    = findpeaks(times);
indchan2 = find(stat_magt2.prob(:,y(5)) <0.025);
clust2   = find(sum(stat_magt2.prob <= 0.025) > 0);

times    = sum(stat_mags.prob <= 0.025);
plot(stat_mags.time,times)
[x,y]    = findpeaks(times);
indchan3 = find(stat_mags.prob(:,y(8)) <0.025);
indchanbest3   = find(sum(stat_mags.prob <= 0.025) > 0);
clust3   = find(sum(stat_mags.prob <= 0.025) > 0);

times    = sum(stat_magi.prob <= 0.025);
plot(stat_magi.time,times)
[x,y]    = findpeaks(times);
indchan4 = find(stat_magi.prob(:,y(3)) <0.025);
indchanbest4   = find(sum(stat_magi.prob <= 0.025) > 0);
clust4   = find(sum(stat_magi.prob <= 0.025) > 0);

times    = sum(stat_magii.prob <= 0.025);
plot(stat_magii.time,times)
[x,y]    = findpeaks(times);
indchan5 = find(stat_magii.prob(:,y(5)) <0.025);
indchanbest5   = find(sum(stat_magii.prob <= 0.025) > 0);
clust5   = find(sum(stat_magii.prob <= 0.025) > 0);

times    = sum(stat_magii2.prob <= 0.005);
plot(stat_magii2.time,times)
[x,y]    = findpeaks(times);
indchan6 = find(stat_magii2.prob(:,y(21)) <0.005);
clust6   = find(sum((stat_magii2.prob <= 0.005).*(stat_magii2.stat > 0) > 0))

times    = sum(stat_magiii2.prob <= 0.005);
plot(stat_magiii2.time,times)
[x,y]    = findpeaks(times);
indchan7 = find(stat_magiii2.prob(:,y(18)) <0.005);
clust7   = find(sum((stat_magiii2.prob <= 0.005).*(stat_magiii2.stat > 0) > 0))
...
plot_lin_shade(clust2, 0.05, stat_magt2,'k',45) % significance window

plot(GDAVGt_magt{1}.time,mean(GDAVG_magt2{1}.avg(indchan2,:)*fmult),'linewidth',8,'color',graphcolor(1,:));hold on % timecourse
plot(GDAVGt_magt{1}.time,mean(GDAVG_magt2{2}.avg(indchan2,:)*fmult),'linewidth',8,'color',graphcolor(2,:));hold on % timecourse

set(gca,'box','off','linewidth',4,'fontsize',40,'fontweight','b')
axis([-0.2 1.2 -50 50])

print('-dpng','/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/PAPER_PLOTS/Mags_Early_Ord_Time_coumap')
...
plot_lin_shade(clust1, 0.05, stat_magt,'k',4.5) % significance window
...
print('-dpng','/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/PAPER_PLOTS/Mags_Early_Ord_Time')
