function TOPOMAP_GROUPlvl_from_mne(niplist,chansel,condnames,latency)
...
folder = '/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/Topomaps_from_mne/';
...
for c = 1:length(GDAVG)
    for tval = latency(1):0.1:latency(2)
        mysubplot(11,length(latency(1):0.1:latency(2)), count)
        cfg                    = [];
        cfg.xlim               = [tval tval];
        cfg.zlim               = zlim;
        cfg.layout             = lay;
...
count = count+length(latency(1):0.1:latency(2));
for c = 1:ndiff
    for tval = latency(1):0.1:latency(2)
        mysubplot(11,length(latency(1):0.1:latency(2)), count)
        cfg                    = [];
