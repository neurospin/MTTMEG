function [tuningcurve_fund, tuningcurve_sha, cfund, csha] = resonance_tuning_SPF(nip,runarray,SPF,SPFname,tag)

freqvalues = [50 75 100 150 200 300 400 600];
r_freqvalues = [50 83.3 100 150 200 300 400 600];
chantype = {'Mags','Grads1','Grads2'};

[x(:,1),y(:,1)] = find(SPF{1} == (ones(102,1)));
[x(:,2),y(:,2)] = find(SPF{2} == (ones(102,1)));
[x(:,3),y(:,3)] = find(SPF{3} == (ones(102,1)));
...
for c = 1:3
    for r = 1:length(runarray)
        for f = 1:length(freqvalues)
            ...
            Powervalue_fund{c,r}(:,f) = FREQ.powspctrm(x(:,c),f_fund_ind);
            ...
            Powervalue_sha{c,r}(:,f) = FREQ.powspctrm(x(:,c),f_sha_ind);
...
tuningcurve_fund{1} = zeros(11,8); tuningcurve_sha{1} = zeros(11,8);
tuningcurve_fund{2} = zeros(11,8); tuningcurve_sha{2} = zeros(11,8);
tuningcurve_fund{3} = zeros(11,8); tuningcurve_sha{3} = zeros(11,8);
