function TLSL_REF(nip,chansel,varargin)
...
if size(varargin,1) >= 2
    for i = 1:size(varargin,1)
        for j = 1:size(varargin{i,1},1)
            datatmp{i,j} = load(['C:\MTT_MEG\data\' nip '\processed\' varargin{i,1}{j,1} '_filt40.mat']);
        end
    end
end
...
for i = 1:length(data)
    ...
    for j = 1:length(data{1,i}.time)
        data{1,i}.time{1,j} = data{1,i}.time{1,j} - ones(1,length(data{1,i}.time{1,j}))*(0.45);
    end
...
% save plots
filename = ['C:\MTT_MEG\results\' nip '\TF_TOPO_'];
for i = 1:size(varargin,1)
    for j = 1:size(varargin{i,1},1)
