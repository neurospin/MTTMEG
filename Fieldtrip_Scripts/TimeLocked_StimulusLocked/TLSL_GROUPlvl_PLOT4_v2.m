function TLSL_GROUPlvl_PLOT4_v2(niplist,chansel_,condclust,condnames,latency,PlotColors)
...
NAMESLIST= {};
NAMESLIST = get_filenames(folder_clust,'posclust',cdn_clust,['Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock'],chansel_);

if isempty(NAMESLIST) == 0
    
    for list = 1:length(NAMESLIST);
        
        load([folder_clust NAMESLIST{list}] );
...
NAMESLIST= {};
NAMESLIST = get_filenames(folder_clust,'negclust',cdn_clust,['Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock'],chansel_);
