function TLSL_GROUPlvl(niplist,chansel,condnames,graphcolor)
...
for i = 1:length(datatmp{1,1}.timelockbase)
    ...
    for j = 1:length(datatmp)
        instr = [instr 'datatmp{1,' num2str(j)  '}.timelockbase{1,' num2str(i) '}{1,1},'];
    end
    instr(end) = [];
    instr      = [instr ');'];
...
if strcmp(statstag,'F') == 1
    instr = 'fig = ERFstatF_GroupLvl_actvsblt(condnames, GDAVG, GDAVGt, chansel,';
else
    instr = 'fig = ERFstatT_GroupLvl_actvsblt(condnames, GDAVG, GDAVGt, chansel,';
end
