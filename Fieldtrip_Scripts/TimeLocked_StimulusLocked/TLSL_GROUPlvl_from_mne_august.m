function TLSL_GROUPlvl_from_mne_august(niplist,chansel,condnames1,condnames2,latency,graphcolor,test)
...
for j = 1:length(niplist)
    datatmp{j}   = load(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/' niplist{j} '/MegData/ERPs_from_mne/' cdn1 chansel],'RegPerChanTime');
    datatmp2{j}   = load(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/' niplist{j} '/MegData/ERPs_from_mne/' cdn2 chansel],'RegPerChanTime');
end
...
for i = 1
    ...
    for j = 1:length(datatmp)
        instr = [instr 'datatmp{1,' num2str(j)  '}.RegPerChanTime,'];
    end
    instr(end) = [];
    instr      = [instr ');'];
...
for i = 2
    ...
    instr = ['GDAVG{' num2str(i) '} = ft_timelockgrandaverage(cfg,'];
    for j = 1:length(datatmp)
        instr = [instr 'datatmp2{1,' num2str(j)  '}.RegPerChanTime,'];
    end
    instr(end) = [];
    instr      = [instr ');'];
