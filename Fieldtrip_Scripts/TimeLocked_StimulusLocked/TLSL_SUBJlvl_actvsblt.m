function TLSL_SUBJlvl_actvsblt(nip,chansel,source,func,segwin,latency)
...
% baseline correction
cfg                        = [];
cfg.baseline           = [latency(1) latency(2)];
cfg.channel            = 'all';
timelockbase{i}       = ft_timelockbaseline(cfg, datalock{i});
%     timelockbaset{i}      = ft_timelockbaseline(cfg, datalockt{i});

cfg          = [];
cfg.toilim = [latency(1) 0];
dataB{i} = ft_redefinetrial(cfg,timelockbase{i});

cfg          = [];
cfg.toilim = [0 latency(2)];
dataA{i} = ft_redefinetrial(cfg,timelockbase{i});

timelockbase{1} = dataA;
timelockbase{2} = dataB;
...
fname = ['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/'...
        nip '/MegData/ERFPs/' condnames{1} 'ACT_' condnames{1} 'BASE' chansel];
