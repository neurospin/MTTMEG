function TLSL_SUBJlvl_cmb(nip,condnames,condarray,segwin,latency,PlotColors)
...
%     cut data 
    ...
    % gradiometers combination
    cfg                          = [];
    cfg.demean             = 'yes';
    cfg.baselinewindow     = [latency(1) 0];
    timelockbasecmbt{i}    = ft_combineplanar(cfg, timelockbaset{i});

    % baseline correction
    cfg                    = [];
    cfg.baseline           = [latency(1) 0];
    timelockbasecmbt{i}    = ft_timelockbaseline(cfg,timelockbasecmbt{i});
    
    timelockbasecmbt{i}.label = Mags';
    ...
    cfg.covariance         = 'yes';
    cfg.vartrllength       = 2;
    timelockbase{i}         = ft_timelockanalysis(cfg, timelockbasecmbt{i}); 
...
lay                      = ft_prepare_layout(cfg,timelockbasecmbt{1});
lay.label              = Mags;
cfg.layout             = lay;
