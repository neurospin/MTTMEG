function TLSL_GROUPlvl_PLOT(niplist,chansel_,condclust,condnames,PlotColors)
...
for condarray = 1:size(condnames,1)
    ...
    GDAVGdiff{condarray} = GDAVG{condarray}{1} ; 
    GDAVGdiff{condarray}.avg = (GDAVGdiff{condarray}.avg ...
        - GDAVG{condarray}{2}.avg);
...
if exist([folder_clust 'stats_posclust_ ' cdn_clust '_' chansel_ '.mat']) == 2
    
    load([folder_clust 'stats_posclust_ ' cdn_clust '_' chansel_ '.mat'] );
        ...
        if isempty(stat.posclusters) == 0
            for k = 1:length(stat.posclusters)
                if stat.posclusters(1,k).prob < 0.07
                    ...
                    instr = [];
                    instr = [instr 'ft_singleplotER(cfg,'];
                    for  condarray = 1:size(condnames,1)

                        instr = [instr 'GDAVGdiff{' num2str(condarray) '},'];
                        
                        % ERF
                        subplot(7,9,loc{condarray})
                        
                        cfg                          = [];
                        cfg.ylim                   = [min([min1 min2  ])*1.5 max([max1 max2 ])*1.2];
                        cfg.graphcolor         = PlotColors;
                        cfg.linewidth            = 3;
                        cfg.channel              = stat.label(linmask);
                        cfg.cm_linestyle        = {'-','-'};
                        ft_singleplotER(cfg,GDAVG{condarray}{1},GDAVG{condarray}{2});
                    ...
                    for  condarray = 1:size(condnames,1)
                        ...
                        subplot(7,9,[28 29 30 37 38 39 46 47 48])
                        ...
                        initplot((condarray-1)*2+1,(condarray-1)*2+1) = mean1{condarray};
                        initplot((condarray-1)*2+2,(condarray-1)*2+2) = mean2{condarray};
                        
                        initplot_sd((condarray-1)*2+1,(condarray-1)*2+1) = sem1{condarray};
                        initplot_sd((condarray-1)*2+2,(condarray-1)*2+2) = sem2{condarray};
                        
                        limmintoplot((condarray-1)*2+1,(condarray-1)*2+1) = initplot((condarray-1)*2+1,(condarray-1)*2+1) ...
                            -  initplot_sd((condarray-1)*2+1,(condarray-1)*2+1);
                        limmintoplot((condarray-1)*2+2,(condarray-1)*2+2) = initplot((condarray-1)*2+2,(condarray-1)*2+2) ...
                            -  initplot_sd((condarray-1)*2+2,(condarray-1)*2+2);
                        limmaxtoplot((condarray-1)*2+1,(condarray-1)*2+1) = initplot((condarray-1)*2+1,(condarray-1)*2+1) ...
                            +  initplot_sd((condarray-1)*2+1,(condarray-1)*2+1);
                        limmaxtoplot((condarray-1)*2+2,(condarray-1)*2+2) = initplot((condarray-1)*2+2,(condarray-1)*2+2) ...
                            +  initplot_sd((condarray-1)*2+2,(condarray-1)*2+2);
                        
                        bar(initplot((condarray-1)*2+1,:),'facecolor',PlotColors(1,:),'linewidth',2);
                        hold on
                        bar(initplot((condarray-1)*2+2,:),'facecolor',PlotColors(2,:),'linewidth',2);
                        hold on
...
elseif exist([folder_clust 'stats_negclust_ ' cdn_clust '_' chansel_ '.mat']) == 2
    
    load([folder_clust 'stats_negclust_ ' cdn_clust '_' chansel_ '.mat'] );
