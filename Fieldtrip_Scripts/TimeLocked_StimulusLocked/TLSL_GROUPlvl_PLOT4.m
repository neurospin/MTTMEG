function TLSL_GROUPlvl_PLOT4(niplist,chansel_,condclust,condnames,latency,PlotColors)
...
for  condarray = 1:size(condnames,1)
    ...
    seg4{condarray} = ft_redefinetrial(cfg,GDAVGt{condarray}{4});
end

if exist([folder_clust 'stats_posclust_ ' cdn_clust '_' chansel_  '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock' '.mat']) == 2
    
    load([folder_clust 'stats_posclust_ ' cdn_clust '_' chansel_  '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock' '.mat'] );
            ...
            for k = 1:length(stat.posclusters)
                if stat.posclusters(1,k).prob < 0.07
                    ...
                    for  condarray = 1:size(condnames,1)
                        ...
                        min4(condarray)  =min(mean(mean(seg4{condarray}.trial(:,y,:),1),2));
                        ...
                        max4(condarray) =max(mean(mean(seg4{condarray}.trial(:,y,:),1),2));
                    end
                    ...
                    for  condarray = 1:size(condnames,1)
                        ...
                        cfg                          = [];
                        cfg.ylim                   = [min([min1 min2 min3 min4])*1.5 max([max1 max2 max3 max4])*1.2];
                        cfg.graphcolor         = PlotColors;
                        cfg.linewidth            = 3;
                        cfg.channel              = stat.label(linmask);
                        cfg.cm_linestyle        = {'-','-','-','-'};
                        ft_singleplotER(cfg,GDAVG{condarray}{1},GDAVG{condarray}{2},GDAVG{condarray}{3},GDAVG{condarray}{4});
...
%                     end
                    ...
                    for  condarray = 1:size(condnames,1)
                        ...
                        mean4{condarray} =mean(mean(mean(seg4{condarray}.trial(:,y,yt))));
                        ...
                        sem4{condarray}   = std(mean(mean(seg4{condarray}.trial(:,y,yt),2),3),0,1)/sqrt(size(seg1{condarray}.trial,1));
                        
                        initplot((condarray-1)*4+1,(condarray-1)*4+1) = mean1{condarray};
                        initplot((condarray-1)*4+2,(condarray-1)*4+2) = mean2{condarray};
                        initplot((condarray-1)*4+3,(condarray-1)*4+3) = mean3{condarray};
                        initplot((condarray-1)*4+4,(condarray-1)*4+4) = mean4{condarray};
                        
                        initplot_sd((condarray-1)*4+1,(condarray-1)*4+1) = sem1{condarray};
                        initplot_sd((condarray-1)*4+2,(condarray-1)*4+2) = sem2{condarray};
                        initplot_sd((condarray-1)*4+3,(condarray-1)*4+3) = sem3{condarray};
                        initplot_sd((condarray-1)*4+4,(condarray-1)*4+4) = sem4{condarray};
                        
                        limmintoplot((condarray-1)*4+1,(condarray-1)*4+1) = initplot((condarray-1)*4+1,(condarray-1)*4+1) ...
                            -  initplot_sd((condarray-1)*4+1,(condarray-1)*4+1);
                        limmintoplot((condarray-1)*4+2,(condarray-1)*4+2) = initplot((condarray-1)*4+2,(condarray-1)*4+2) ...
                            -  initplot_sd((condarray-1)*4+2,(condarray-1)*4+2);
                        limmintoplot((condarray-1)*4+3,(condarray-1)*4+3) = initplot((condarray-1)*4+3,(condarray-1)*4+3) ...
                            -  initplot_sd((condarray-1)*4+3,(condarray-1)*4+3);
                        limmintoplot((condarray-1)*4+4,(condarray-1)*4+4) = initplot((condarray-1)*4+4,(condarray-1)*4+4) ...
                            -  initplot_sd((condarray-1)*4+4,(condarray-1)*4+4);
                        limmaxtoplot((condarray-1)*4+1,(condarray-1)*4+1) = initplot((condarray-1)*4+1,(condarray-1)*4+1) ...
                            +  initplot_sd((condarray-1)*4+1,(condarray-1)*4+1);
                        limmaxtoplot((condarray-1)*4+2,(condarray-1)*4+2) = initplot((condarray-1)*4+2,(condarray-1)*4+2) ...
                            +  initplot_sd((condarray-1)*4+2,(condarray-1)*4+2);
                        limmaxtoplot((condarray-1)*4+3,(condarray-1)*4+3) = initplot((condarray-1)*4+3,(condarray-1)*4+3) ...
                            +  initplot_sd((condarray-1)*4+3,(condarray-1)*4+3);
                        limmaxtoplot((condarray-1)*4+4,(condarray-1)*4+4) = initplot((condarray-1)*4+4,(condarray-1)*4+4) ...
                            +  initplot_sd((condarray-1)*4+4,(condarray-1)*4+4);
                        
                        bar(initplot((condarray-1)*4+1,:),'facecolor',PlotColors(1,:),'linewidth',2);
                        hold on
                        bar(initplot((condarray-1)*4+2,:),'facecolor',PlotColors(2,:),'linewidth',2);
                        hold on
                        bar(initplot((condarray-1)*4+3,:),'facecolor',PlotColors(3,:),'linewidth',2);
                        hold on
                        bar(initplot((condarray-1)*4+4,:),'facecolor',PlotColors(4,:),'linewidth',2);
                        hold on
                    ...
                    elseif size(condnames,1)==4
                        filename = [folder cdn_clust '/PlotposClust_' chansel_ '_posclust' num2str(countpos) '_VS_' cdn{1} cdn{2} cdn{3} cdn{4}];
                    end
                    print('-dpng',filename)
...
elseif exist([folder_clust 'stats_negclust_ ' cdn_clust '_' chansel_  '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock' '.mat']) == 2
    
    load([folder_clust 'stats_negclust_ ' cdn_clust '_' chansel_  '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock' '.mat'] );
...
%                     end
                    ...
                    elseif size(condnames,1)==4
                        filename = [folder cdn_clust '/PlotnegClust_' chansel_ '_negclust' num2str(countneg) '_VS_' cdn{1} cdn{2} cdn{3} cdn{4}];
                    end
                    print('-dpng',filename)
