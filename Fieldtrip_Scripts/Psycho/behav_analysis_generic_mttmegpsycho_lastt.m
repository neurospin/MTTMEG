...
for r = 1:length(REPETITION)
    ...
    for i = 1:36 % mini-block (= ref)
        if REPETITION{1,r}.REF(i) == 1
            for j = 2:REPETITION{r}.EVENTNUM(i)
                count = 1;
                while count <= 36
...
print('-dpng',['C:\MTT_MEG\results\FULL_RT_MT_MEG_PSYCHO_last_' NIP])
...
print('-dpng',['C:\MTT_MEG\results\MEAN_RT_MT_MEG_PSYCHO_last_' NIP])
...
print('-dpng',['C:\MTT_MEG\results\REGRESS_TIME_RT_MT_MEG_PSYCHO_last_' NIP])
...
print('-dpng',['C:\MTT_MEG\results\REGRESS_SPACE_RT_MT_MEG_PSYCHO_last_' NIP])
