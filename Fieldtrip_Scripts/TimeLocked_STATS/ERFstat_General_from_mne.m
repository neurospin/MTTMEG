function ERFstat_General_from_mne(condnames,latency,GDAVG,GDAVGt, chansel_,graphcolor,stat_test,varargin)
    ...
    if isempty(stat.posclusters) == 0
        for k = 1:length(stat.posclusters)
            if stat.posclusters(1,k).prob < 0.07 % permissive treshold to look for marginal effects
                ...
                save(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/spatial_clust_from_mne/stats_posclust_' stat_test '_' num2str(countpos) '_' cdn '_' chansel_ '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock_' timetag '.mat'],'stat','chansel','condnames')
    ...
    if isempty(stat.negclusters) == 0
        for k = 1:length(stat.negclusters)
            if stat.negclusters(1,k).prob < 0.07 % permissive treshold to look for marginal effects
                ...
                save(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/spatial_clust_from_mne/stats_negclust_' stat_test '_'  num2str(countneg) '_' cdn '_' chansel_ '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock_' timetag '.mat'],'stat','chansel','condnames')
