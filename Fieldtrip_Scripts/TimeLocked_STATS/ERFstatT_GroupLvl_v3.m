function fig = ERFstatT_GroupLvl_v3(condnames,latency,GDAVG,GDAVGt, chansel_, data1,data2,graphcolor)
    ...
    if isempty(stat.posclusters) == 0
        for k = 1:length(stat.posclusters)
            if stat.posclusters(1,k).prob < 0.07
                ...
                cfg                          = [];
                cfg.ylim                   = [min([min1 min2])*1.2 max([max1 max2 ])*1.2];
...
%                 legend(condnames{1},condnames{2},condnames{3},condnames{4},'Location','NorthWest')
                ...
                axis([ 0 4 min([mean1-sem1 mean2-sem2  ])*1.2 ...
                    max([mean1+sem1 mean2+sem2  ])*1.5]);
                set(gca,'xtick',1:2,'xticklabel',{''}, 'box','off','linewidth',2,'fontsize',14,'fontweight','b');
                xticklabel_rotate([1 2 ],45,{condnames{1},condnames{2}},'Fontsize',12,'fontweight','b')
