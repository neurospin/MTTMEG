function fig = ERFstatT_GroupLvl_INT_v3(condnames1,condnames2,latency,GDAVG1,GDAVG1t,GDAVG2,GDAVG2t,chansel_,graphcolor)
    ...
    if isempty(stat.posclusters) == 0
        for k = 1:length(stat.posclusters)
            if stat.posclusters(1,k).prob < 0.07
                ...
                cfg                          = [];
                cfg.ylim                   = [min([min1 min2  ])*1.5 max([max1 max2 ])*1.5];
                cfg.graphcolor         = [mean(graphcolor(1:2,:));mean(graphcolor(3:4,:)); graphcolor];
                cfg.linewidth            = 3;
                cfg.channel              = stat.label(linmask);
                cfg.cm_linestyle        = {'-','-','-','-','.-','.-'};
                ft_singleplotER(cfg,GDAVG1save{1},GDAVG2save{1},GDAVG1save{2},GDAVG2save{2},GDAVG1{1},GDAVG2{1});
                ...
                hold on
                bar([0 0 mean1b 0],'facecolor',graphcolor(3,:),'linewidth',2);
                hold on
                bar([0 0 0 mean2b],'facecolor',graphcolor(4,:),'linewidth',2);
                %                 legend(condnames{1},condnames{2},'Location','NorthWest')
                errorbar([mean1 mean2 mean1b mean2b ],[sem1 sem2 sem1b sem2b],'linestyle','none','color','k','linewidth',2);hold on
                ylabel(['Amplitude (' ampunit ')'],'fontsize',14,'fontweight','b');
                axis([ 0 5 min([mean1-sem1 mean2-sem2])*1.2 ...
                    max([mean1+sem1 mean2+sem2 ])*1.5]);
    ...
    if isempty(stat.negclusters) == 0
        for k = 1:length(stat.negclusters)
            if stat.negclusters(1,k).prob < 0.07
                ...
                cfg                          = [];
                cfg.ylim                   = [min([min1 min2  ])*1.5 max([max1 max2 ])*1.5];
                cfg.graphcolor         = [graphcolor mean(graphcolor(1:2,:));mean(graphcolor(3:4,:))];
                cfg.linewidth            = 3;
                cfg.channel              = stat.label(linmask);
                cfg.cm_linestyle        = {'-','-','-','-','.-','.-'};
                ft_singleplotER(cfg,GDAVG1save{1},GDAVG2save{1},GDAVG1save{2},GDAVG2save{2},GDAVG1{1},GDAVG2{1});
                ...
                set(gcf,'Color',[1,1,1])
                bar([mean1 0],'facecolor',graphcolor(1,:),'linewidth',2);
                hold on
                bar([0 mean2],'facecolor',graphcolor(2,:),'linewidth',2);
%                 legend(condnames{1},condnames{2},'Location','NorthWest')
                errorbar([mean1 mean2 ],[sem1 sem2 ],'linestyle','none','color','k','linewidth',2);hold on
                ylabel(['Amplitude (' ampunit ')'],'fontsize',14,'fontweight','b');
                axis([ 0 3 min([mean1-sem1 mean2-sem2])*1.2 ...
                    max([mean1+sem1 mean2+sem2 ])*1.5]);
                set(gca,'xtick',1:2,'xticklabel',{''}, 'box','off','linewidth',2,'fontsize',14,'fontweight','b');
