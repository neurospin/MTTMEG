function fig = ERFstatT_GroupLvl_INT_from_mne(condnames1,condnames2,GDAVG1,GDAVG1t,GDAVG2,GDAVG2t,chansel_,latency,graphcolor,stat_test)
    ...
    if isempty(stat.posclusters) == 0
        for k = 1:length(stat.posclusters)
            if stat.posclusters(1,k).prob < 0.07
                ...
                for i = 1:length(seg)
                    mean_(i)  = mean(mean(mean(seg{i}.trial(:,y,yt+offset-1))));
                    sem_(i)   = std(mean(mean(seg{1}.trial(:,y,yt+offset-1),2),3),0,1)/sqrt(size(seg{1}.trial,1));
                end
                barm = diag(mean_);
                barsd = diag(sem_);
                ...
                axis([ 0 (length(seg)+1) min([mean_ -sem_])*1.2 max([mean_ +sem_])*1.5]);
                set(gca,'xtick',1:length(seg),'xticklabel',{''}, 'box','off','linewidth',2,'fontsize',14,'fontweight','b');
                
                % rotate instruction
                instr = 'xticklabel_rotate([1:length(seg)],45,{';
                for i =1:length(seg)
                ...
                for i = 1:length(seg)
                    mean_(i)  = mean(mean(mean(seg{i}.trial(:,indchanx,yt+offset-1))));
                    sem_(i)   = std(mean(mean(seg{1}.trial(:,indchanx,yt+offset-1),2),3),0,1)/sqrt(size(seg{1}.trial,1));
                end
                barm = diag(mean_);
                barsd = diag(sem_);
                ...
                folder = '/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/ERFPs_from_mne/';
                if exist([folder cdn],'dir') == 0
                    mkdir([folder cdn])
                end
                % save plots
                filename = [folder cdn '/STATS_' stat_test '_' chansel_ '_posclust' num2str(countpos) '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock_' timetag];
                print('-dpng',filename)
                
                save(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/spatial_clust_from_mne/stats_posclust_' stat_test '_'  num2str(countpos) '_' cdn '_' chansel_ '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock_' timetag '.mat'],'stat','chansel','condnames1','condnames2')
    ...
    if isempty(stat.negclusters) == 0
        for k = 1:length(stat.negclusters)
            if stat.negclusters(1,k).prob < 0.07
                ...
                % save plots
                filename = [folder cdn '/STATS_' stat_test '_' chansel_ '_negclust' num2str(countneg) '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock_' timetag];
                print('-dpng',filename)
                
                save(['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/spatial_clust_from_mne/stats_negclust_' stat_test '_'  num2str(countneg) '_' cdn '_' chansel_ '_Lat' num2str(latency(1)) '-' num2str(latency(2)) '_stimlock_' timetag '.mat'],'stat','chansel','condnames1','condnames2')
