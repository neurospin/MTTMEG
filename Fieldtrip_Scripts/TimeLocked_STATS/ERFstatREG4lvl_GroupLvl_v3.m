function count = ERFstatREG4lvl_GroupLvl_v3(condnames,latency,GDAVG,GDAVGt, chansel_, data1,data2,data3,data4,graphcolor)
    ...
    if isempty(stat.posclusters) == 0
        for k = 1:length(stat.posclusters)
            if stat.posclusters(1,k).prob < 0.07
                ...
                min4 = min(mean(mean(seg4.trial(:,indchanx,:),1),2));
                ...
                max4 =max(mean(mean(seg4.trial(:,indchanx,:),1),2));
               ...
               % Cluster RASTER PLOT Tvalues
                ...
                mean4 =mean(mean(mean(seg4.trial(:,y,yt+offset))));
                ...
                sem4   = std(mean(mean(seg4.trial(:,y,yt+offset),2),3),0,1)/sqrt(size(seg1.trial,1));
...
%                 legend(condnames{1},condnames{2},condnames{3},condnames{4},'Location','NorthWest')
                ...
                set(gca,'xtick',1:4,'xticklabel',{''}, 'box','off','linewidth',2,'fontsize',14,'fontweight','b');
                xticklabel_rotate([1 2 3 4],45,{condnames{1},condnames{2},condnames{3},condnames{4}},'Fontsize',12,'fontweight','b')
                ...
                mean4 =mean(mean(mean(seg4.trial(:,indchanx,yt+offset))));
                ...
                sem4   = std(mean(mean(seg4.trial(:,indchanx,yt+offset),2),3),0,1)/sqrt(size(seg1.trial,1));
