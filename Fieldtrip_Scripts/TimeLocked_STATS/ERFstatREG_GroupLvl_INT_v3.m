function fig = ERFstatREG_GroupLvl_INT_v3(condnames1,condnames2,condnames3,latency,GDAVG1,GDAVG1t,GDAVG2,GDAVG2t,GDAVG3,GDAVG3t,chansel_,graphcolor)
    ...
    if isempty(stat.posclusters) == 0
        for k = 1:length(stat.posclusters)
            if stat.posclusters(1,k).prob < 0.07
                ...
                set(fig,'PaperPosition',[1 1 900 900])
                ...
                loc = {[1 2 3 10 11 12 19 20 21];[4 5 6 13 14 15 22 23 24];...
                ...
                cfg.cm_linestyle        = {'-','-','--'};
                ft_singleplotER(cfg,GDAVG1save{1},GDAVG1save{2});
                ...
                title([texlabel(cdn_{1},'literal')],'fontsize',14,'fontweight','b');
                ...
                cfg.channel              = stat.label(linmask);
                cfg.cm_linestyle        = {'-','-','--'};
                ft_singleplotER(cfg,GDAVG2save{1},GDAVG2save{2});
                %                     hold on;line([GDAVG1{1}.time(1) GDAVG1{1}.time(end)],[0 0],'color','k')
                linenim = stat.time(linmaskt);
                line([linenim(1) linenim(end)],[ min([min1 min2 ])*1.1 min([min1 min2  ])*1.1],'linewidth',2,'color','k')
                title([texlabel(cdn_{2},'literal')],'fontsize',14,'fontweight','b');
                xlabel('Time (s)','fontsize',14,'fontweight','b');
                set(gca, 'box','off','linewidth',2,'fontsize',14,'fontweight','b');
                %                 legend(condnames,'location','EastOutside')
                ...
                cfg.channel              = stat.label(linmask);
                cfg.cm_linestyle        = {'-','-','--'};
                ft_singleplotER(cfg,GDAVG3save{1},GDAVG3save{2});
                %                     hold on;line([GDAVG1{1}.time(1) GDAVG1{1}.time(end)],[0 0],'color','k')
                linenim = stat.time(linmaskt);
                line([linenim(1) linenim(end)],[ min([min1 min2 ])*1.1 min([min1 min2  ])*1.1],'linewidth',2,'color','k')
                title([texlabel(cdn_{3},'literal')],'fontsize',14,'fontweight','b');
                xlabel('Time (s)','fontsize',14,'fontweight','b');
                set(gca, 'box','off','linewidth',2,'fontsize',14,'fontweight','b');
                %                 legend(condnames,'location','EastOutside')
                
                % ERF
                subplot(9,9,[37 38 39 46 47 48 55 56 57])
                ...
                cfg.channel              = stat.label(linmask);
                cfg.cm_linestyle        = {'-','-','-'};
                ft_singleplotER(cfg,GDAVG1{1},GDAVG2{1},GDAVG3{1});
                hold on;line([GDAVG1{1}.time(1) GDAVG1{1}.time(end)],[0 0],'color','k')
                linenim = stat.time(linmaskt);
                line([linenim(1) linenim(end)],[ min([min1 min2 ])*1.1 min([min1 min2  ])*1.1],'linewidth',2,'color','k')
                title([texlabel(cdn_{3},'literal')],'fontsize',14,'fontweight','b');
                ...
                % RASTER PLOT Tvalues
                subplot(9,9,[43 44 45 52 53 54 61 62 63])
