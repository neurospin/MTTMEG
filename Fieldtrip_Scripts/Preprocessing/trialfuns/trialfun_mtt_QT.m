function [trl] = trialfun_mtt_QT(cfg)
    ...
    elseif  isempty(intersect(k,[1024 32768])) == 0
        for i = 1:length(MiniBlock.PasPar)
            ...
            for xi = 1:length(x)
                ...
                elseif (x(xi)-3)>0 && isempty(intersect(MiniBlock.PasPar{1,i}(x(xi)-3,2),9)) == 0; % spacequestion
                    if isempty(intersect(MiniBlock.PasPar{1,i}(x(xi),2),1024)) == 0; % leftbutton
    ...
    for t2 = 1:length(recodetrig)
        if isempty(intersect(samplecorr(t2),samplecorr(x{t}))) == 0
            ...
            if (samplecorr(t2) - cfg.trialdef.prestim(t)*hdr.Fs + cfg.photodelay(t)*hdr.Fs) >= 0
                trltmp(count,1) = samplecorr(t2) - cfg.trialdef.prestim(t)*hdr.Fs + cfg.photodelay(t)*hdr.Fs;
                trltmp(count,2) = samplecorr(t2) + cfg.trialdef.poststim(t)*hdr.Fs + cfg.photodelay(t)*hdr.Fs;
