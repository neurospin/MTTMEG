...
x = []; y = [];
[x17,y17] = find(valuetrigcorr == 17);
[x21,y21] = find(valuetrigcorr == 21);
[x25,y25] = find(valuetrigcorr == 25);
[x29,y29] = find(valuetrigcorr == 29);
[x33,y33] = find(valuetrigcorr == 33);

x = [x17 x21 x25 x29 x33];
y = [y17 y21 y25 y29 y33];

for i = 1:length(x)
    trl(i,1) = samplecorr(x(i)) - cfg.trialdef.prestim*hdr.Fs + cfg.photodelay;
    trl(i,2) = samplecorr(x(i)) + cfg.trialdef.poststim*hdr.Fs + cfg.photodelay;
    trl(i,3) = 0;
end
