function [trl] = trialfun_mtt_for_mne(cfg)

%% test dataset
% cfg.dataset = '/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/MEG/sd130343/raw_sss/run2_GD_trans_sss.fif';
...
%% to match mne values
sample = sample - ones(length(sample),1)*sample(1);
...
triggerlist1 = [1:5 6:2:15 7:2:15 16:4:32 17:4:33 18:4:34 19:4:35 ]; % dist/event original triggers
...
for t = 1: length(triggerlist1)
    ...
    for t2 = 1:length(recodetrig)
        if isempty(intersect(samplecorr(t2),samplecorr(x{t}))) == 0
            ...
            count= 1;
            if (samplecorr(t2) >= 0)
                trltmp(count,1) = samplecorr(t2)  ;
                trltmp(count,2) = samplecorr(t2)  ;
...
% trl(:,7:8) = datamatch(1:length(trl),:);
% disp('test')
