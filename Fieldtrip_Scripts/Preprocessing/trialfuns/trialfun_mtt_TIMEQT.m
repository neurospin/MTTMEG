...
%% test dataset
% cfg.dataset = 'C:\MTT_MEG\data\sd130343\run1_GD_trans_sss.fif';
...
[x6,y6]   = find(valuetrig == 6);
[x8,y8]   = find(valuetrig == 8);
[x10,y10] = find(valuetrig == 10);
[x12,y12] = find(valuetrig == 12);
[x14,y14] = find(valuetrig == 14);

x         = [x6 x8 x10 x12 x14];
y         = [y6 y8 y10 y12 y14];

for i = 1:length(x)
    trl(i,1) = sample(y(i)) - cfg.trialdef.prestim*hdr.Fs + cfg.photodelay;
    trl(i,2) = sample(y(i)) + cfg.trialdef.poststim*hdr.Fs + cfg.photodelay;
    trl(i,3) = 0;
end
