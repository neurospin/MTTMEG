...
triggerlist = [1:5 6:2:14 7:2:15 16:4:32 17:4:33 18:4:34 19:4:35];
...
for t = 1: length(triggerlist)
    ...
    for i = 1:length(x{t})
        if (samplecorr(x{t}(i)) - cfg.trialdef.prestim(t)*hdr.Fs + cfg.photodelay(t)*hdr.Fs) >= 0
            trltmp(count,1) = samplecorr(x{t}(i)) - cfg.trialdef.prestim(t)*hdr.Fs ;
            trltmp(count,2) = samplecorr(x{t}(i)) + cfg.trialdef.poststim(t)*hdr.Fs ;
            trltmp(count,4) = t;
