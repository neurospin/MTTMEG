function [condnames,dataset] = make_AVG_QTall_EVS(datasource,rejectvisual)
...
%% condames
condnames = {'QsWestEVS';'QsEastEVS';'QsParEVS';'QsPastEVS';'QsFutEVS'};

%% trial selection
select_event{1} = [TABLE{9,2}(:) ];
select_event{2} = [TABLE{10,2}(:) ];
select_event{3} = [TABLE{6,2}(:) ];
select_event{4} = [TABLE{7,2}(:) ];
select_event{5} = [TABLE{8,2}(:) ];
