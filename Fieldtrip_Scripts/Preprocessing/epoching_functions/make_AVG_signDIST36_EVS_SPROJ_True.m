function [condnames,dataset,dist_val,tag] = make_AVG_signDIST36_EVS_SPROJ_True(datasource,rejectvisual)
...
tag = 'SignEVSdistS_Sproj'

%% select events
dist_val = all_distS;

select_event = [];
for i = 1:length(all_distS)/2
    select_event{i} = all_EVS((2*i-1):(2*i));
end

%% condames
for i = 1:length(select_event)
    condnames{i} = {['sDS_Sproj' num2str(dist_val(i))]}
end
