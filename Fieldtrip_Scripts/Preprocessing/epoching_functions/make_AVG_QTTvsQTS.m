function [condnames,dataset] = make_AVG_QTTvsQTS(datasource,rejectvisual)
...
if isfield(datatmp{1,1},'datafilt40')
...
elseif isfield(datatmp{1,1},'data')
    if length(datasource) >= 2
        datatmp{1,1}.data         = ft_appenddata([],datatmp{1,1}.data,datatmp{1,2}.data);
        datatmp{1,1}.trldef        = [datatmp{1,1}.trldef ;datatmp{1,2}.trldef ];
        rejecttmp{1,1}.trlsel       = [rejecttmp{1,1}.trlsel rejecttmp{1,2}.trlsel ];
    end
end
