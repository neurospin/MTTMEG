function [condnames,dataset] = make_AVG_QTshifts_EVS(datasource,rejectvisual)
...
%% condames
condnames = {'QsSpaceShiftEVS';'QsNoShiftEVS';'QsTimeShiftEVS'};

%% trial selection
select_event{1} = [TABLE{9,2}(:) ; TABLE{10,2}(:) ];
select_event{2} = [TABLE{6,2}(:) ];
select_event{3} = [TABLE{7,2}(:) ; TABLE{8,2}(:) ];
