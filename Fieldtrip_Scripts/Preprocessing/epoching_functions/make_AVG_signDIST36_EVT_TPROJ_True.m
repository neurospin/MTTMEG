function [condnames,dataset,dist_val,tag] = make_AVG_signDIST36_EVT_TPROJ_True(datasource,rejectvisual)
...
all_distT = [ tmp2(:) ; tmp3(:)   ]
all_EVT   = [ EVT_Pas(:) ; EVT_Fut(:)];

tag = 'SignEVTdistT_Tproj'
...
for i = 1:length(select_event)
    condnames{i} = {['sDT_Tproj' num2str(dist_val(i))]}
end
