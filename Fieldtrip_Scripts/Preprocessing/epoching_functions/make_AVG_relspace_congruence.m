function [condnames,dataset] = make_AVG_relspace_congruence(datasource,rejectvisual)
...
% Relative West irrespective of other conditions
TABLE{11,2}   = repmat([37:42 43:48 49:54]*1000,1,1) + ones(1,18)*70   + repmat([3],1,18);
TABLE{12,2}   = repmat([37:40 43:46 49:52]*1000,1,1) + ones(1,12)*90   + repmat([3],1,12);
TABLE{13,2}   = repmat([39:42 45:48 51:54]*1000,1,1) + ones(1,12)*110   + repmat([3],1,12);
TABLE{14,2}   = repmat([37:42 43:48]*1000,1,1) + ones(1,12)*130   + repmat([3],1,12);
TABLE{15,2}   = repmat([49:54 55:60]*1000,1,1) + ones(1,12)*150   + repmat([3],1,12);

% Relative East irrespective of other conditions
TABLE{16,2}   = repmat([55:60 61:66 67:72]*1000,1,1) + ones(1,18)*70   + repmat([1],1,18);
TABLE{17,2}   = repmat([55:58 61:64 67:70]*1000,1,1) + ones(1,12)*90   + repmat([1],1,12);
TABLE{18,2}   = repmat([57:60 63:66 69:72]*1000,1,1) + ones(1,12)*110   + repmat([1],1,12);
TABLE{19,2}   = repmat([49:54 55:60]*1000,1,1) + ones(1,12)*130   + repmat([1],1,12);
TABLE{20,2}   = repmat([61:66 67:72]*1000,1,1) + ones(1,12)*150   + repmat([1],1,12);
