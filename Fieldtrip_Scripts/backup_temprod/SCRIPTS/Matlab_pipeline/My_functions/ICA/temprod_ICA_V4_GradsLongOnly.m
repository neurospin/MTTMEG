function temprod_ICA_V4_GradsLongOnly(index,subject,freqband,method,numcomponent,show)
...
freqcomppath           = [par.ProcDataDir '/FT_ICs/runica-comp' num2str(numcomponent) 'V4_GradsLongOnly_freq' num2str(index) '.mat'];
...
if show == 1
    for i = 1:numcomponent
        ...
        mysubplot(8,8,i*4 - 3)
        cfg.comment = [subject ' r' num2str(index) 'Gradslat comp' num2str(i)];
        cfg.electrodes = 'off';
...
print('-dpng',['/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_' subject...
    '/ICAV4_GradslongOnly_topo_overview_' num2str(freqband(1)) '-' num2str(freqband(2)) '_' num2str(index) '_' num2str(numcomponent) 'comp.png']);
...
if show == 1
    ...
    ylabel('log Power')
    xlabel('frequency (Hz)')
    title([subject ' run : ' num2str(index) 'Gradslong Components Mean power Spectrm averaged across trials'])
...
print('-dpng',['/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_' subject...
    '/ICAV4_GradsLongOnly_overview_' num2str(freqband(1)) '-' num2str(freqband(2)) '_' num2str(index) '_' num2str(numcomponent) 'comp.png']);
...
for i = 1:numcomponent
    ...
    subplot(4,5,i)
    plot(freq.freq,log(squeeze(mean(freq.powspctrm(:,i,:),1))),'linewidth',2,'color',C(i,:));
    axis([0 80 -10 -2]);
    xlabel('frequency');
    ylabel('log power');
...
print('-dpng',['/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_' subject...
    '/ICAV4_GradsLongOnly_individual_comps_' num2str(freqband(1)) '-' num2str(freqband(2)) '_' num2str(index) '_' num2str(numcomponent) 'comp.png']);
