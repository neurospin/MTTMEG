function temprod_FreqPow_corr_allbands_V1(index,subject)
    ...
    for jind = 1:3
        ...
        for i = 1:size(Fullspctrm,1)
            ...
            while C(j) <= C(end)/2
                ...
                Fpeakpow(i)          =  mean(mean(squeeze((Fullspctrm(i,:,Fpeak(i,:))))));
                j                    =  j+1;
            end
        end
        MaxPSD     = Fpeakpow;
...
print('-dpng',['/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_' subject...
    '/FullcorrallbandsV1_' num2str(freqband(1)) '-' num2str(freqband(2)) '_' num2str(index) '.png']);
