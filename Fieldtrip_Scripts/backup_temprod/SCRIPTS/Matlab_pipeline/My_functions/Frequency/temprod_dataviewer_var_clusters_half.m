function temprod_dataviewer_var_clusters_half(indexes,subject,freqband,debiasing,loglogdetrend,datatoplot,mode,savetag)
...
%             Fullfreq(LNfband)       = [];
            ...
            if strcmp(datatoplot,'MEAN')
                plot(Fullfreq,log(MEANspctrm1),'color',colors_bis{k},'linewidth',1);
                hold on
                plot(Fullfreq,log(MEANspctrm2),'color','r','linewidth',1,'linestyle','-');
            end
            
            if strcmp(datatoplot,'STD')
                plot(Fullfreq,log(STD1),'color',colors_bis{k},'linewidth',1);
                hold on
                plot(Fullfreq,log(STD2),'color','r','linewidth',1,'linestyle','-');
            end
            
            title([clustname ' ' chantypefull{j} ' ' subject ' : intertrial power ' datatoplot ])
            xlabel('frequency (Hz)')
            ylabel('log-power')
...
if savetag == 1
    print('-dpng',['/neurospin/meg/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_' subject...
        '/VAR_spectra_clusters_half_' datatoplot '_' num2str(indexes) '_' ...
