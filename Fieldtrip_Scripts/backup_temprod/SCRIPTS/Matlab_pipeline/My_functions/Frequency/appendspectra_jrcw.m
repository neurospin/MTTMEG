function appendspectra_jrcw(indexrun,subject,freqband,tag)
...
load(fullfile(Dir,['/FT_trials/run_jrcw_' num2str(indexrun) 'trial001.mat']))
...
for j = 1:3
    ...
    for x               = 1:ntrials
        ...
        trialdataset{x}    = ['/FT_spectra/' chantype 'freq_jrcw_' num2str(freqband(1)) '_' num2str(freqband(2)) 'run' num2str(indexrun) 'trial' num2str(x) '.mat'];
    ...
    disp(['trials concatenated in Fullspctrm_jrcw_' chantype num2str(indexrun)])
    
    Fullspctrm_path     = [par.ProcDataDir 'FT_spectra/Fullspctrm_jrcw_' chantype num2str(indexrun) '.mat'];
