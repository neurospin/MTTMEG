function [M,allchan,badchannels] = computeprojmatrix_onselectedchannels(pcapath ,samplefile, projfile_id)

% Compute the matrix to project PCA on the data
%
% pcapath : the path of the directory your projection are
% chan_file   : an evoked fif file with data that contains the list of the channel 

proj_id = dir([pcapath '*' subj '*.fif']);

% load the PCA projection
for pf = 1:length(proj_id)
    [fid, tree, directory] = fiff_open([pcapath proj_id(pf).name]);
...
info = fiff_read_meas_info(samplefile);
