function datapath = temprod_preproc_ICA_EEG(run,isdownsample,subject,runref,rejection,badEEG)
...
MaxLength(i)                = (max(cfg_loc.trl(:,2) - cfg_loc.trl(:,1)))/data.fsample;
...
durinfopath = [par.ProcDataDir 'FT_trials/runICA_EEG' num2str(run) 'durinfo.mat'];
...
for i = 1:length(datasave.time)
    ...
    datapath               = [par.ProcDataDir 'FT_trials/runICA_EEG' num2str(run) 'trial' num2str(i,'%03i') '.mat'];
