function data_corr = cwbg_correct_ecgeog(cfg,corrtag)
...
switch corrtag
    case 'ecg_only'
        ...
        if ~isfield(cfg,'ecgchan'),       cfg_ecg.ecgchan     = find(cell2mat(cellfun(@(x) ~isempty(findstr('ECG',x)), hdr.label, 'UniformOutput', false))); end
    ...
    case 'eog_only'
        ...
        if ~isfield(cfg,'eogchan'),       cfg_eog.ecgchan     = find(cell2mat(cellfun(@(x) ~isempty(findstr('EOG061',x)), hdr.label, 'UniformOutput', false))); end
