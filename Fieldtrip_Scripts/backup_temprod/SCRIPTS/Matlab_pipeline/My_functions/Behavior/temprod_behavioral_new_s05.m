...
%% set parameters %%
DataDir             = '/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Trans_sss_s05/';
RunNum              = [1 2 3];
fsample             = 1000;

%% main script
for i               = RunNum
    DATASETS{i}     = [DataDir 's05_run' num2str(RunNum(i)) '_raw_sss.fif'];
...
TD = [6.5 8.5 6.5];         % targeted durations
...
subplot(4,4,14); plot([1 2 1],meAn(RunNum),'marker','o','linestyle','none'); ylabel('mean'); axis([0 3 4 10]);
hold on 
errorbar([1 2 1],meAn(RunNum),SD(RunNum),'linestyle','none')
subplot(4,4,15); plot([1 2 1],meAnnorm(RunNum),'marker','o','linestyle','none'); ylabel('std/mean'); axis([0 3 0 0.4])
...
print('-r0','-dpng',['/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_s05/behavioral.png']);
