function [Pxx] = temprod_BehaviorFFT_RT_v5(subjectArray,RunNum,fsample,tag)
...
subplot(2,4,1); plot(D); Title('successive TE values');ylabel('duration estimates(s)');

% get histogram profile
T = max(D) - min(D);
subplot(2,4,2); hist(D,T/0.01); title('TE histogram'); % 10ms bins
[n1,xout1] = hist(D,(T/0.01)); % 25ms bins = length(s) * 40(hz)
...
subplot(2,4,6)
plot(Pxx,'linewidth',2)
xlabel('Frequency (Hz)')
ylabel('PSD')

if length(subjectArray) == 1
    print('-r0','-dpng',['C:\Users\bgauthie\Desktop\RESONANCE_MEG_OLD_RESULTS\NEW_DATA_2012/behavioralFFT_pwelch' subjectArray{1} '.png']);
end
