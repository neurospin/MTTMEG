...
%% set parameters %%
DataDir             = '/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Trans_sss_s07/';
RunNum              = [1 2 3 4 5 6];
fsample             = [2000 2000 1000 1000 1000 1000];

%% main script
for i               = RunNum
    DATASETS{i}     = [DataDir 's07_run' num2str(RunNum(i)) '_raw_sss.fif'];
...
FULLDUR = [DURATIONS_norm{1} DURATIONS_norm{2} DURATIONS_norm{3} DURATIONS_norm{4}];
...
[n4,m4] = hist(DURATIONS_norm{4},length(DURATIONS_norm{4})/4);
[n5,m5] = hist(DURATIONS_norm{5},length(DURATIONS_norm{5})/5);
[n6,m6] = hist(DURATIONS_norm{6},length(DURATIONS_norm{6})/6);

nmin = min([n1,n2,n3,n4]);
mmin = min([m1,m2,m3,n4]);
nmax = max([n1,n2,n3,n4]);
mmax = max([m1,m2,m3,n4]);

CI = [1 2 3 4 5 6];                % ascending order of targeted durations
TD = [6.5 8.5 6.5 6.5 8.5 8.5];         % targeted durations
...
for i = RunNum
    subplot(4,6,i)
    hist(DURATIONS_corr{CI(i)}/fsample(i),length(DURATIONS_norm{CI(i)})/3)
%     axis([3 15 0 15]);
...
%     end
    xlabel(['Target = ' num2str(TD(i)) 's, median = ' num2str(median(DURATIONS_corr{CI(i)}/fsample(i))) 's'])
end

%% plot normalized histograms and gamma fit
for i                   = 1:length(RunNum)
    subplot(4,6,i+6)
    [n_in_bins,nb_bins] = hist(DURATIONS_corr{CI(i)}/fsample(i),length(DURATIONS_norm{CI(i)})/3);
...
subplot(4,6,14); plot([1 2 3 1 2 4],meAn(1:6),'marker','o','linestyle','none'); ylabel('mean'); axis([0 5 4 10]);
hold on 
errorbar([1 2 3 1 2 4],meAn(1:6),SD(1:6),'linestyle','none')
subplot(4,6,16); plot([1 2 3 1 2 4],meAnnorm(1:6),'marker','o','linestyle','none'); ylabel('std/mean'); axis([0 5 0 0.4])
subplot(4,6,15); plot([1 2 3 1 2 4],SD(1:6),'marker','o','linestyle','none'); ylabel('std'); axis([0 5 0 2])
...
for i = RunNum
    ...
    hold on
    plot((DURATIONS_sum{i}(PlotRanges{i}))/fsample(i),(DURATIONS_corr{i})/fsample(i),'linewidth',2,'color','r')
%     axis([1 (max([max(DURATIONS_sum{1}) length(DURATIONS_sum{2}) length(DURATIONS_sum{3}) length(DURATIONS_sum{4})]))/fsample...
%     3 12]);
    ylabel('duration')
    xlabel('acquisition time')
end

print('-r0','-dpng',['/neurospin/meg_tmp/temprod_Baptiste_2010/DATA/NEW/Plots_s06/behavioral.png']);
