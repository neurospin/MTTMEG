...
T = max(D) - min(D);
subplot(2,4,5); hist(D,length(D)); title('TE histogram');
[n1,xout1] = hist(D,length(D));
subplot(2,4,6); plot(xout1,n1); title('histogram profile');

% compute and apply high-pass filter
[b,a] = butter(3,0.05,'high');
n1f = filter(b,a,n1);
subplot(2,4,7); plot(xout1,n1f); title('filtered profile'); 
...
if length(subjectArray) == 1
    print('-r0','-dpng',[root '\DATA\NEW/Plots_' subjectArray{1} '/behavioralFFT.png']);
end
