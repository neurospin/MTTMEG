function TFSL_PLOT_GROUPlvl(niplist,chansel,condnames,latency)
...
folder = '/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/GROUP/TFplots/';
if exist([folder cdn],'dir') == 0
    mkdir([folder cdn])
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for c = 1:length(condnames)
    ...
    % save plots
    filename = [folder cdn '/TF_lowfreq_mean_' condnames{c} '_' chansel];
    print('-dpng',filename)
...
if length(condnames) == 2
    ...
    % save plots
    filename = [folder cdn '/TF_lowfreq_mean_' condnames{1} '-' condnames{2} '_' chansel];
    print('-dpng',filename)
...
elseif length(condnames) == 3
    ...
    % save plots
    filename = [folder cdn '/TF_lowfreq_mean_' condnames{3} '-' condnames{2} '_' chansel];
    print('-dpng',filename)
