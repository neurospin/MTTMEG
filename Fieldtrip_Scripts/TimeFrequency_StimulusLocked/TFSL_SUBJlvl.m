function TFSL_SUBJlvl(nip,chansel,condnames,condarray,segwin,latency)
    ...
    for i = 1:size(condarray,2)
        for j = 1:size(condarray{1,i},1)
            datatmp{j,i} = load (['/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/From_laptop/Subjects/'...
                nip '/MegData/Processed/' condarray{1,i}{j,1} tag 'clean.mat'],'dataclean');
...
% cfg.keeptrials       = 'no';
% cfg.foi                  = 1:1:30;
% cfg.toi                  =  (latency(1)):0.025:(latency(2));
% cfg.width              = linspace(2,7,length(1:1:30));
% cfg.gwidth            = 3;
