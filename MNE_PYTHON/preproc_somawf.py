...
initbody = initbody + "import Epoch_per_run as Epr\n"
...
for s,subj in enumerate(ListSubj):
    for m,mod in enumerate(modalities):
        for c,cond in enumerate(CondComb):
            ...
            body = initbody + "Epr.Epoch_per_run(" + str(CondComb[c])        
            ...
            name_file = []
            name_file = os.path.join(path_script, ("JOBS_PREPROC/Preproc_STC1" + jobname + ".py"))
...
for i in range(len(Listfile)):
    ...
    somaWF_name = os.path.join(path_script, "SOMA_WFs/soma_WF_PREPROC_allsub_allcond")
