# -*- coding: utf-8 -*-
"""
Created on Thu Apr 14 17:11:48 2016
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 20 10:49:24 2016

@author: bgauthie
"""

def Write_covmat_icaclean(datasource,subject,runpersubject, badeeg, meg_tag, eeg_tag):
    ...
    def Epoch(wdir, raw, SelEve, event_id, epoch_tmin, epoch_tmax, baseline_tmin, baseline_tmax, meg_tag, eeg_tag):
        
        # epoch data (I do not decimat the baseline in order to get more data to compute covariance)
        picks      = mne.pick_types(raw.info, meg=meg_tag,  eeg=eeg_tag, stim=True , eog=False, include=[], exclude='bads')
    ...
    def CovCompute(wdir, baseline, baseline_tmin, baseline_tmax):
    ...
    def EpochsAppend(wdir, epoch_list):
...
#    eeg_tag        = True
    ...
    for j,run in enumerate(runpersubject):
        ...
        baseline   = Epoch(wdir, raw, SelEve, None, -0.2, 1.1, -0.2, 0, meg_tag, eeg_tag) 
        #epochs_list.append(epochs)
        baseline_list_subj.append(baseline)
        del raw, SelEve
        
        # match the number of trial between conditions per run and per subject
    baseline2pool = [baseline_list_subj[e] for e in range(len( baseline_list_subj))]
        # compute covariance on trialnb-matched epochs
    CatEpochs= []
    CatEpochs = mne.epochs.concatenate_epochs(baseline2pool) 
    ...
    noisecov_tot = CovCompute(wdir, CatEpochs, -0.2, 0)

    noisecov_tot.save(wdir + subject + "/mne_python/COVMATS/" + modality +"_noisecov_icacorr_" +
