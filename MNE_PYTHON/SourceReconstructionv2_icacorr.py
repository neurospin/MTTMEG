# -*- coding: utf-8 -*-
"""
Created on Fri Apr 15 17:21:02 2016

@author: bgauthie
"""

def SourceReconstructionv2(condnames,ListSubj,modality,Method,covmatsource):
    ...
    for c in range(len(condnames)):
        for i in range(len(ListSubj)):
            ...
            fname_epochs      = (wdir+ListSubj[i]+"/mne_python/EPOCHS/MEEG_epochs_icacorr_" +condnames[c]+ '_' + ListSubj[i]+"-epo.fif")          
    ...
    ###############################
	    ...
	    # MEG source reconstruction
    	    ...
    	    stccond1.save(wdir+ListSubj[i]+"/mne_python/STCS/IcaCorr_"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_oriNone_"+Method+"_ico-5-fwd.fif")

    	    stccond1norm = apply_inverse(evokedcond1, inverse_operator1, lambda2,method =Method, pick_ori= "normal")
    	    stccond1norm.save(wdir+ListSubj[i]+"/mne_python/STCS/IcaCorr_"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_orinormal_"+Method+"_ico-5-fwd.fif")

            # morphing to fsaverage
    	    stc_fsaverage_cond1 = mne.morph_data(ListSubj[i], 'fsaverage', stccond1,smooth = 20)
    	    stc_fsaverage_cond1.save(wdir+ListSubj[i]+"/mne_python/STCS/IcaCorr_"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_oriNone_"+Method+"_ico-5-fwd-fsaverage.fif")

    	    stc_fsaverage_cond1norm = mne.morph_data(ListSubj[i], 'fsaverage', stccond1norm,smooth = 20)
    	    stc_fsaverage_cond1norm.save(wdir+ListSubj[i]+"/mne_python/STCS/IcaCorr_"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_orinormal_"+Method+"_ico-5-fwd-fsaverage.fif")
    ...
    ###################
            ...
            stccond1.save(wdir+ListSubj[i]+"/mne_python/STCS/"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_oriNone_"+Method+"_ico-5-fwd.fif")

            stccond1norm = lcmv(evokedcond1, forward, NOISE_COV1,DATA_COV1, reg=0.01,pick_ori= "normal")
            stccond1norm.save(wdir+ListSubj[i]+"/mne_python/STCS/"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_orinormal_"+Method+"_ico-5-fwd.fif")

            # morphing to fsaverage
            stc_fsaverage_cond1 = mne.morph_data(ListSubj[i], 'fsaverage', stccond1,smooth = 20)
            stc_fsaverage_cond1.save(wdir+ListSubj[i]+"/mne_python/STCS/"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_oriNone_"+Method+"_ico-5-fwd-fsaverage.fif") 

            stc_fsaverage_cond1norm = mne.morph_data(ListSubj[i], 'fsaverage', stccond1norm,smooth = 20)
            stc_fsaverage_cond1norm.save(wdir+ListSubj[i]+"/mne_python/STCS/"+modality+"_"+ListSubj[i]+"_"+condnames[c]+"_pick_orinormal_"+Method+"_ico-5-fwd-fsaverage.fif")   
