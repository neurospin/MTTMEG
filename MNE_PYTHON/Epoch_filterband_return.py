# -*- coding: utf-8 -*-
"""
Created on Thu Oct 29 18:48:27 2015

@author: bgauthie
"""

def Epoch_filterband_return(conditions,datasource,subject,runpersubject, badeeg, Filt):
    ...
    def Trldef(wdir, condition, datasource, subject, run, runind, badEEG, Filt):
        ...
        raw.filter(l_freq=Filt, h_freq=30, method = 'iir', n_jobs=1)
...
#############################################################################################################
    ...
    for c,cond in enumerate(conditions):
        ...
        epoch_tot.save(wdir + subject + "/mne_python/EPOCHS/MEEG_epochs_" +
         conditions[c] + "_" + subject + "_LP" + str(Filt) + "Hz-epo.fif")
