...
def Decode2cond(subject, condcombs, modality):
    ...
    for c,comb in enumerate(itertools.combinations(condcombs,2)):  
        ...
        PlotDir = ('/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/MEG/GROUP/decoding_context_yousra/MVPA_time/Plots/icacorr' 
        + '_vs_'.join([str(cond) for cond in comb]) + '/')  
        
        ScoreDir = ('/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/MEG/GROUP/decoding_context_yousra/MVPA_time/Scores/icacorr' 
        + '_vs_'.join([str(cond) for cond in comb]) + '/')  
        ...
        plt.savefig(PlotDir + subject + "_" + modality +'icacorr' )
        plt.close()
        
        Scores = np.array(scores)
        np.save(ScoreDir + subject + "_" + modality +'icacorr' ,Scores)
