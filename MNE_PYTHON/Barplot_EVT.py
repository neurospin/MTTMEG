# -*- coding: utf-8 -*-
"""
Created on Wed Jan  4 14:51:06 2017
...
condition  = ('EtPast','EtPre','EtFut','EsWest','EsPar','EsEast')
mod        = 'MEEG'
method     = 'dSPM'
...
#    VertOI = np.where(np.abs(stc_label.data) >0)[0] 
    ...
    # get the indexes of vertices of interest (VertOI)
    path = '/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/MEG/GROUP/mne_python/Plot_STATS/EtPast_vs_EtPre_vs_EtFut';
    stc_fname = (path + '/fmapMEEG_EtPast_vs_EtPre_vs_EtFut-rh.stc')
    stc = mne.read_source_estimate(stc_fname)
    stc_label = stc.in_label(label) 
    path2 = '/neurospin/meg/meg_tmp/MTT_MEG_Baptiste/MEG/GROUP/mne_python/Plot_STATS/EsWest_vs_EsPar_vs_EsEast';
    stc_fname2 = (path2 + '/fmapMEEG_EsWest_vs_EsPar_vs_EsEast-rh.stc')
    stc2 = mne.read_source_estimate(stc_fname2)
    stc_label2 = stc2.in_label(label) 
    VertOI = np.where((np.abs(stc_label.data) >5.25)*(np.abs(stc_label2.data) > 5.25))[0] 
    ...
    ax1.plot((0,0),(0.5,5.5),linewidth = 3,linestyle='--',color = (0.5,0.5,0.5))
    ax1.plot((1,1),(0.5,5.5),linewidth = 3,linestyle='--',color = (0.5,0.5,0.5))
    for c,cond in enumerate(condition):
    ...
    data4 = np.mean(np.mean(AVG_STC_crop[VertOI,:,:,3],axis = 0),axis = 0)
    data5 = np.mean(np.mean(AVG_STC_crop[VertOI,:,:,4],axis = 0),axis = 0)
    data6 = np.mean(np.mean(AVG_STC_crop[VertOI,:,:,5],axis = 0),axis = 0)

    DATA.append(np.hstack([data1, data2,data3,data4, data5,data6]))
    SUBJECT.append(np.hstack([range(1,20),range(1,20),range(1,20),range(1,20),range(1,20),range(1,20)]))
    COND.append(np.hstack([np.ones(19),np.ones(19)*2,np.ones(19)*3,np.ones(19)*4,np.ones(19)*5,np.ones(19)*6]))
    ROI.append(np.hstack([np.ones(19)*(l+1),np.ones(19)*(l+1),np.ones(19)*(l+1),np.ones(19)*(l+1),np.ones(19)*(l+1),np.ones(19)*(l+1)]))
